---
title: 一致性协议(一) Gossip 设计与实现
categories: 分布式
date: 2020-12-26 22:49:20
keywords: 分布式, Gossip, 一致性
tags: [分布式, Gossip, 一致性]
---

近期忙完 Lua 的垃圾回收改造后，非常沉迷于分布式的设计。虽然在我看来，游戏服务端几乎不需要分布式，甚至最好不要用分布式。不过在游戏服务端的部分服务采用分布式的设计还是很不错的，比如说服务发现，数据存储这两块地方使用。

说到服务发现，就不得不提一下，现在市面主流的服务发现基本上都是基于 CP 实现的，即一致性优先的原则，我不知道为什么他们都这样做，一旦网络出现波动，很有可能服务发现整套系统都会玩完，或许他们是家大业大，公司的机房牛逼？（小公司根本不可能有这么稳的网络，就算是租用的云服务，~~也不会被云服务厂商重视~~）

扯远了，这是一致性协议的第一篇，为什么选择 Gossip 作为起点，主要有以下几点组成。

1. 简单
2. 基于 AP，可用性优先（毕竟基于一致性优先的开源服务都这么多了，反复造轮子也不合适）

## Gossip 简述

当我还在读大学的时候，因为学校实在是太无聊了，没事我就会坐车回家，出现在小区的次数就比较多。有一天走在路上，听到小区里的大妈和其他大妈说，这个小孩是不是被退学了，怎么天天出现在这。久而久之，我家小区就都知道我“被”学校退学了。

Gossip协议的大致思路就像上面那样，通过谣言传播的形式，传播给其他节点。
...
广播应该都好理解，反熵就不是那么好理解了，首先吐槽一下这个熵字对广东人实在太不友好了，怎么念怎么别扭。熵指的是杂乱无章，反熵你懂得。

反熵其实就是通过 推 拉，推拉的形式，将两个节点的数据进行交换，进而达成一致。之所以有了广播还要有反熵去推拉，是因为有可能缓存区满了，丢了数据，或者是一个新节点刚刚上线，它肯定就没办法得到之前广播出来的消息啦，那就需要反熵进行修复。
...
主要分析 [memberlist](https://github.com/hashicorp/memberlist) 的实现，其依赖于 Gossip 的变种，SWIM。
