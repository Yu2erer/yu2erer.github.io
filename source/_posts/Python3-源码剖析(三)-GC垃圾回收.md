...
## 想法
### 渐进分代？
`CPython` 的GC是 `Stop The World` 的，哪怕它已经很尽力用分代的方式去减少GC的损耗。是否可以将其改进为渐进的方式？我目前的想法是在容器操作时，进行 `Barrier` 操作，维护一个中间态，使得前面的扫描过程是可渐进的，最后处理垃圾的时候再停下来一次性处理完，减少停止的时间。

### 减少跟踪对象？
是否可以对其它常用容器也做 `untrack` 操作，当容器没有嵌套容器时，取消 `track` 操作，减少GC遍历损耗？
